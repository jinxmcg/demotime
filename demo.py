...
def test(filenames):
    ...
    #wait from logs mongo to be up MongoDB init process complete; 
    while "ready for start up" not in logs.decode("utf-8"):
        time.sleep(0.1)
        logs = subprocess.check_output(cmdline)


    #create test database and set some values
    mongo = pymongo.MongoClient("mongodb://"+os.getenv('MONGO_INITDB_ROOT_USERNAME')+":"+os.getenv('MONGO_INITDB_ROOT_PASSWORD')+"@"+os.getenv('MONGO_INITDB_HOSTNAME')+":"+os.getenv('MONGO_INITDB_PORT')+"/")    
    mydb  = mongo[os.getenv('MONGO_INITDB_DATABASE')]
    mydb.command("createUser", os.getenv('MONGO_INITDB_USERNAME'), pwd=os.getenv('MONGO_INITDB_PASSWORD'),roles=[{'role':'readWrite','db':os.getenv('MONGO_INITDB_DATABASE')}])

    print(mongo.list_database_names())

    print("Creating Test db:"+os.getenv('MONGO_INITDB_DATABASE'));
